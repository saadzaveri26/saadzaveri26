<div align="center">

<!-- ═══════════════════════════════════════════════════════════════════════ -->
<!-- HERO SECTION -->
<!-- ═══════════════════════════════════════════════════════════════════════ -->

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:0d1117,30:fe428e,70:a855f7,100:00d4ff&height=230&section=header&text=Muhammed%20Saad%20Zaveri&fontSize=44&fontColor=ffffff&fontAlignY=34&desc=Full%20Stack%20Developer%20%7C%20AI%20Engineer%20%7C%20AI%20and%20Data%20Science&descSize=17&descColor=a9fef7&descAlignY=56&animation=twinkling" width="100%" />

<!-- Animated Typing Banner -->
<a href="https://git.io/typing-svg">
  <img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=600&size=26&duration=3000&pause=1000&color=FE428E&center=true&vCenter=true&multiline=true&repeat=true&width=900&height=100&lines=%F0%9F%9A%80+Building+Intelligent+Systems+That+Matter;%F0%9F%A7%A0+Next.js+%7C+TypeScript+%7C+Python+%7C+Agentic+Workflows;%E2%9A%A1+LangChain+%7C+LangGraph+%7C+Vercel+AI+SDK+%7C+RAG;%F0%9F%94%90+Cybersecurity+Minor+%7C+AI+%26+Data+Science+Undergrad" alt="Typing SVG" />
</a>

<br/>

<!-- Visitor Counter & Profile Badges -->
<p>
  <img src="https://komarev.com/ghpvc/?username=saadzaveri26&label=Profile+Views&color=fe428e&style=for-the-badge&logo=github" alt="Profile Views" />
  &nbsp;
  <a href="https://github.com/saadzaveri26?tab=followers">
    <img src="https://img.shields.io/github/followers/saadzaveri26?label=Followers&style=for-the-badge&color=a855f7&logo=github&logoColor=white" alt="Followers" />
  </a>
  &nbsp;
  <a href="https://github.com/saadzaveri26?tab=repositories">
    <img src="https://img.shields.io/badge/Public%20Repos-Active-00d4ff?style=for-the-badge&logo=github&logoColor=white" alt="Repos" />
  </a>
  &nbsp;
  <img src="https://img.shields.io/badge/Open%20Source-%E2%9D%A4-ff007c?style=for-the-badge&logo=opensourceinitiative&logoColor=white" alt="Open Source" />
</p>

<!-- Social Proof -->
<p>
  <a href="https://linkedin.com/in/muhammed-saad-zaveri-771875292">
    <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn" />
  </a>
  <a href="mailto:zaverisaad26@gmail.com">
    <img src="https://img.shields.io/badge/Gmail-EA4335?style=for-the-badge&logo=gmail&logoColor=white" alt="Gmail" />
  </a>
  <a href="https://leetcode.com/Saadzaveri26">
    <img src="https://img.shields.io/badge/LeetCode-FFA116?style=for-the-badge&logo=leetcode&logoColor=black" alt="LeetCode" />
  </a>
  <a href="https://github.com/saadzaveri26">
    <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white" alt="GitHub" />
  </a>
</p>

<img src="https://user-images.githubusercontent.com/73097560/115834477-dbab4500-a447-11eb-908a-139a6edaec5c.gif" width="100%" />

</div>

<!-- ═══════════════════════════════════════════════════════════════════════ -->
<!-- ABOUT ME -->
<!-- ═══════════════════════════════════════════════════════════════════════ -->

## <img src="https://media.giphy.com/media/hvRJCLFzcasrR4ia7z/giphy.gif" width="28"> &nbsp;About Me

```yaml
name: Muhammed Saad Zaveri
located_in: Mumbai, Maharashtra, India
education:
  degree: "Bachelors of Engineering in Artificial Intelligence and Data Science"
  minor: "Cybersecurity"
  institution: "Rizvi College of Engineering"
  graduation: "2027"

current_focus:
  - "Designing Agentic AI & Multi-Agent systems using LangChain and LangGraph"
  - "Building fully functional in-browser development tools (WebContainers, Convex)"
  - "Optimizing Retrieval-Augmented Generation (RAG) pipelines for enterprise data"
  - "Evaluating and benchmarking frontier AI models for reasoning and factuality"

philosophy: >
  I bridge the gap between robust full-stack software development and artificial intelligence.
  I believe in architecting secure, scalable, and highly interactive systems that put
  advanced AI models directly into the workflows of developers and enterprises.

areas_of_specialization:
  - "Generative AI & LLM Workflows"
  - "Full-Stack Web Engineering (Next.js, Node.js)"
  - "Agentic Workflows & Multi-Agent Orchestration"
  - "Vector Search & Retrieval-Augmented Generation"
  - "Cybersecurity & Secure Application Architecture"
  - "Real-time Data Processing & Analytics"
```

<br/>

<div align="center">
<img src="https://user-images.githubusercontent.com/73097560/115834477-dbab4500-a447-11eb-908a-139a6edaec5c.gif" width="100%" />
</div>

<!-- ═══════════════════════════════════════════════════════════════════════ -->
<!-- EXPERIENCE -->
<!-- ═══════════════════════════════════════════════════════════════════════ -->

## 💼 &nbsp;Experience

### 🔬 &nbsp;AI Generalist Expert (Independent Contractor)
**Mercor.io** | *March 2026 – May 2026* | *Remote*

- **Frontier AI Systems Evaluation:** Evaluated and refined frontier AI systems for a leading AI research lab, focusing on model performance in real-world search and complex multi-step reasoning tasks.
- **Quality & Factuality Assessments:** Conducted deep-dive factuality and quality assessments of AI-generated responses, providing structured feedback to improve model accuracy, helpfulness, and completeness.
- **LLM Benchmarking:** Developed technical rationales for side-by-side model comparisons, identifying recurring failure modes and edge cases to benchmark LLM performance.

<br/>

<div align="center">
<img src="https://user-images.githubusercontent.com/73097560/115834477-dbab4500-a447-11eb-908a-139a6edaec5c.gif" width="100%" />
</div>

<!-- ═══════════════════════════════════════════════════════════════════════ -->
<!-- PROJECTS -->
<!-- ═══════════════════════════════════════════════════════════════════════ -->

## 🚀 &nbsp;Featured Projects

<div align="center">

<table>
<tr>

<td width="50%" valign="top">

### 💻 SideKick — In-Browser IDE & AI Pair Programmer
<br/>
<p>
<img src="https://img.shields.io/badge/Next.js-000000?style=flat-square&logo=nextdotjs&logoColor=white"/>
<img src="https://img.shields.io/badge/Convex-000000?style=flat-square&logo=convex&logoColor=white"/>
<img src="https://img.shields.io/badge/TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white"/>
<img src="https://img.shields.io/badge/Inngest-FF5A1F?style=flat-square&logo=inngest&logoColor=white"/>
<img src="https://img.shields.io/badge/Vercel_AI_SDK-000000?style=flat-square&logo=vercel&logoColor=white"/>
</p>

An advanced, fully functional code editor built for the modern web. Engineered with Next.js and Convex, it leverages WebContainers to run Node.js natively in the browser alongside an integrated terminal.

**Key Architecture & Features:**
- **In-Browser Runtime:** Leverages WebContainers to execute Node.js programs and run server commands directly inside standard browser environments.
- **AI Pair Programming:** Uses Vercel AI SDK and Inngest workflows to embed multi-model AI agents that assist with code writing, debugging, and execution on the fly.
- **Real-Time Collaboration:** Enabled robust multi-developer co-editing, AI agent interactions, and secure live-session sharing using Convex's real-time state sync.

</td>

<td width="50%" valign="top">

### 🤖 AskRCOE — Multimodal AI Assistant
<br/>
<p>
<img src="https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white"/>
<img src="https://img.shields.io/badge/LangChain-1C3C3C?style=flat-square&logo=langchain&logoColor=white"/>
<img src="https://img.shields.io/badge/LangGraph-000000?style=flat-square"/>
<img src="https://img.shields.io/badge/FAISS-000000?style=flat-square"/>
<img src="https://img.shields.io/badge/Google_Gemini-8E75C2?style=flat-square&logo=googlegemini&logoColor=white"/>
</p>

An intelligent institutional RAG pipeline and assistant designed to query complex institutional knowledge bases across multiple modalities.

**Key Architecture & Features:**
- **High-Accuracy RAG:** Engineered with LangChain and LangGraph to orchestrate complex RAG pipelines, achieving **99.2% retrieval accuracy**.
- **Sub-Second Semantic Search:** Implemented FAISS-based indexing for over 10K+ institutional documents, delivering sub-second search latencies.
- **Multimodal & Multilingual:** Built audio processing pipelines supporting voice-to-text and text-to-speech conversion with multilingual support across 15+ languages.

</td>

</tr>
<tr>

<td width="100%" colspan="2" valign="top">

### 📊 Synapse-AI-Arena — Interactive ML Platform
<br/>
<p>
<img src="https://img.shields.io/badge/Streamlit-FF4B4B?style=flat-square&logo=streamlit&logoColor=white"/>
<img src="https://img.shields.io/badge/Scikit--Learn-F7931E?style=flat-square&logo=scikitlearn&logoColor=white"/>
<img src="https://img.shields.io/badge/XGBoost-000000?style=flat-square"/>
<img src="https://img.shields.io/badge/Plotly-3F4F75?style=flat-square&logo=plotly&logoColor=white"/>
<img src="https://img.shields.io/badge/Pandas-150458?style=flat-square&logo=pandas&logoColor=white"/>
</p>

An interactive machine learning platform orchestrating complex data pipelines and model testing in real-time.

**Key Architecture & Features:**
- **Pipeline Orchestration:** Built inside Streamlit to run complex feature engineering and train Scikit-Learn / XGBoost models dynamically.
- **High-Dimensional Visualizations:** Integrated Plotly, Altair, and Pydeck to visualize and explore massive datasets of 100K+ samples in real time.
- **Rapid Prototyping:** Streamlined model iterations from hours to minutes via automated data cleaning and automated feature generation pipelines.

</td>

</tr>
</table>

</div>

<br/>

<div align="center">
<img src="https://user-images.githubusercontent.com/73097560/115834477-dbab4500-a447-11eb-908a-139a6edaec5c.gif" width="100%" />
</div>

<!-- ═══════════════════════════════════════════════════════════════════════ -->
<!-- ENGINEERING STACK -->
<!-- ═══════════════════════════════════════════════════════════════════════ -->

## ⚡ &nbsp;Engineering Stack

<div align="center">

### 💻 Programming Languages
<p>
<img src="https://skillicons.dev/icons?i=python,javascript,typescript,html,css,bash&theme=dark" />
</p>

### 🎨 Frontend Development
<p>
<img src="https://skillicons.dev/icons?i=react,nextjs,tailwind,vite,figma&theme=dark" />
</p>

### ⚙️ Backend & Database
<p>
<img src="https://skillicons.dev/icons?i=nodejs,express,postgres,sqlite&theme=dark" />
&nbsp;
<img src="https://img.shields.io/badge/Convex-000000?style=for-the-badge&logo=convex&logoColor=white"/>
</p>

### 🤖 AI / ML / Data Science
<p>
<img src="https://img.shields.io/badge/LangChain-1C3C3C?style=for-the-badge&logo=langchain&logoColor=white"/>
<img src="https://img.shields.io/badge/LangGraph-000000?style=for-the-badge"/>
<img src="https://img.shields.io/badge/Vercel_AI_SDK-000000?style=for-the-badge&logo=vercel&logoColor=white"/>
<img src="https://img.shields.io/badge/Google_Gemini-8E75C2?style=for-the-badge&logo=googlegemini&logoColor=white"/>
<img src="https://img.shields.io/badge/OpenRouter-000000?style=for-the-badge"/>
<img src="https://img.shields.io/badge/Scikit_Learn-F7931E?style=for-the-badge&logo=scikitlearn&logoColor=white"/>
<img src="https://img.shields.io/badge/XGBoost-000000?style=for-the-badge"/>
</p>
<p>
<img src="https://img.shields.io/badge/Pandas-150458?style=for-the-badge&logo=pandas&logoColor=white"/>
<img src="https://img.shields.io/badge/NumPy-013243?style=for-the-badge&logo=numpy&logoColor=white"/>
<img src="https://img.shields.io/badge/FAISS-000000?style=for-the-badge"/>
</p>

### ⚙️ Automation & Workflows
<p>
<img src="https://img.shields.io/badge/n8n-FF6C37?style=for-the-badge&logo=n8n&logoColor=white"/>
<img src="https://img.shields.io/badge/Inngest-FF5A1F?style=for-the-badge&logo=inngest&logoColor=white"/>
<img src="https://img.shields.io/badge/Apify-FF0000?style=for-the-badge&logo=apify&logoColor=white"/>
<img src="https://img.shields.io/badge/Webhooks-000000?style=for-the-badge"/>
</p>

### 🔐 Security
<p>
<img src="https://img.shields.io/badge/OWASP-000000?style=for-the-badge&logo=owasp&logoColor=white"/>
<img src="https://img.shields.io/badge/Cybersecurity_Minor-000000?style=for-the-badge"/>
</p>

### ⚡ Tools & Platforms
<p>
<img src="https://skillicons.dev/icons?i=git,github,vscode,postman,anaconda&theme=dark" />
</p>

</div>

<br/>

<div align="center">
<img src="https://user-images.githubusercontent.com/73097560/115834477-dbab4500-a447-11eb-908a-139a6edaec5c.gif" width="100%" />
</div>

<!-- ═══════════════════════════════════════════════════════════════════════ -->
<!-- COMPETITIVE PROGRAMMING & ACHIEVEMENTS -->
<!-- ═══════════════════════════════════════════════════════════════════════ -->

## <img src="https://media2.giphy.com/media/QssGEmpkyEOhBCb7e1/giphy.gif?cid=ecf05e47a0n3gi1bfqntqmob8g9aid1oyj2wr3ds3mg700bl&rid=giphy.gif" width="28"> &nbsp;Competitive Programming & Achievements

<div align="center">

<table>
<tr>
<td align="center" width="25%">

<a href="https://leetcode.com/Saadzaveri26">
<img src="https://img.shields.io/badge/LeetCode-FFA116?style=for-the-badge&logo=leetcode&logoColor=black" alt="LeetCode" />
</a>
<br/>
<sub><b>Problem Solving</b></sub>
<br/>
<sub>Data Structures & Algorithms</sub>

</td>
<td align="center" width="25%">

<img src="https://img.shields.io/badge/CodeChef-5B4638?style=for-the-badge&logo=codechef&logoColor=white" alt="CodeChef" />
<br/>
<sub><b>Algorithmic Coding</b></sub>
<br/>
<sub>Problem Solving</sub>

</td>
<td align="center" width="25%">

<img src="https://img.shields.io/badge/GeeksforGeeks-0F9D58?style=for-the-badge&logo=geeksforgeeks&logoColor=white" alt="GFG" />
<br/>
<sub><b>Core Practice</b></sub>
<br/>
<sub>DSA & CS Concepts</sub>

</td>
<td align="center" width="25%">

<img src="https://img.shields.io/badge/HackerRank-00EA64?style=for-the-badge&logo=hackerrank&logoColor=black" alt="HackerRank" />
<br/>
<sub><b>Verified Skills</b></sub>
<br/>
<sub>Algorithms & Language Fundamentals</sub>

</td>
</tr>
</table>

<!-- LeetCode Stats Card -->
<a href="https://leetcode.com/Saadzaveri26">
  <img src="https://leetcard.jacoblin.cool/Saadzaveri26?theme=dark&font=JetBrains%20Mono&ext=heatmap&border=0&radius=12" alt="LeetCode Stats" width="500"/>
</a>

</div>

<br/>

<div align="center">
<img src="https://user-images.githubusercontent.com/73097560/115834477-dbab4500-a447-11eb-908a-139a6edaec5c.gif" width="100%" />
</div>

<!-- ═══════════════════════════════════════════════════════════════════════ -->
<!-- ENGINEERING COMPETENCIES -->
<!-- ═══════════════════════════════════════════════════════════════════════ -->

## 🏗️ &nbsp;Engineering Experience & Competencies

<div align="center">

<table>
<tr>
<td align="center" width="20%"><b>🤖</b><br/><sub>Agentic<br/>Workflows</sub></td>
<td align="center" width="20%"><b>🔍</b><br/><sub>RAG<br/>Pipelines</sub></td>
<td align="center" width="20%"><b>⚡</b><br/><sub>Real-time Data<br/>Processing</sub></td>
<td align="center" width="20%"><b>🧬</b><br/><sub>Generative AI<br/>& LLMs</sub></td>
<td align="center" width="20%"><b>📊</b><br/><sub>Machine<br/>Learning</sub></td>
</tr>
<tr>
<td align="center" width="20%"><b>🌐</b><br/><sub>Next.js &<br/>React</sub></td>
<td align="center" width="20%"><b>⚙️</b><br/><sub>API<br/>Integrations</sub></td>
<td align="center" width="20%"><b>🔐</b><br/><sub>Application<br/>Security</sub></td>
<td align="center" width="20%"><b>🔗</b><br/><sub>In-Browser<br/>Runtimes</sub></td>
<td align="center" width="20%"><b>🛠️</b><br/><sub>Workflow<br/>Automation</sub></td>
</tr>
</table>

</div>

<br/>

<div align="center">
<img src="https://user-images.githubusercontent.com/73097560/115834477-dbab4500-a447-11eb-908a-139a6edaec5c.gif" width="100%" />
</div>

<!-- ═══════════════════════════════════════════════════════════════════════ -->
<!-- PROFESSIONAL HIGHLIGHTS -->
<!-- ═══════════════════════════════════════════════════════════════════════ -->

## 🏆 &nbsp;Professional Highlights

<div align="center">

```
 ╔══════════════════════════════════════════════════════════════════╗
 ║                                                                  ║
 ║   🤖 Evaluated frontier LLMs for leading AI research labs        ║
 ║   🧠 Engineered RAG assistant with 99.2% retrieval accuracy       ║
 ║   💻 Built in-browser code editor running Node.js via Convex     ║
 ║   🛡️ Advanced studies in Cybersecurity (Minors/Specialization)   ║
 ║   ⛓️ Developed agentic workflow systems using LangChain & Graph  ║
 ║   🏆 Accomplished competitive programmer on LeetCode & GFG       ║
 ║                                                                  ║
 ╚══════════════════════════════════════════════════════════════════╝
```

</div>

<br/>

<div align="center">
<img src="https://user-images.githubusercontent.com/73097560/115834477-dbab4500-a447-11eb-908a-139a6edaec5c.gif" width="100%" />
</div>

<!-- ═══════════════════════════════════════════════════════════════════════ -->
<!-- GITHUB METRICS -->
<!-- ═══════════════════════════════════════════════════════════════════════ -->

## 📈 &nbsp;GitHub Metrics

<div align="center">

<!-- GitHub Stats & Top Languages -->
<p>
  <a href="https://github.com/saadzaveri26">
    <img width="49%" src="https://github-readme-stats-eight-theta.vercel.app/api?username=saadzaveri26&show_icons=true&theme=radical&hide_border=true&border_radius=12&include_all_commits=true&count_private=true" alt="GitHub Stats" />
  </a>
  <a href="https://github.com/saadzaveri26">
    <img width="49%" src="https://github-readme-stats-eight-theta.vercel.app/api/top-langs/?username=saadzaveri26&layout=compact&theme=radical&hide_border=true&border_radius=12&langs_count=10" alt="Top Languages" />
  </a>
</p>

<!-- GitHub Streak -->
<p>
  <a href="https://github.com/saadzaveri26">
    <img width="60%" src="https://streak-stats.demolab.com/?user=saadzaveri26&theme=radical&hide_border=true&border_radius=12" alt="GitHub Streak" />
  </a>
</p>

<!-- Activity Graph -->
<p>
  <a href="https://github.com/saadzaveri26">
    <img width="98%" src="https://github-readme-activity-graph.vercel.app/graph?username=saadzaveri26&theme=rogue&hide_border=true&radius=12&area=true" alt="Activity Graph" />
  </a>
</p>

<!-- GitHub Trophies -->
<p>
  <a href="https://github.com/saadzaveri26">
    <img width="98%" src="https://github-trophies.vercel.app/?username=saadzaveri26&theme=radical&no-frame=true&no-bg=true&column=8&margin-w=10&margin-h=10" alt="GitHub Trophies" />
  </a>
</p>

<!-- Profile Summary Cards -->
<p>
  <a href="https://github.com/saadzaveri26">
    <img width="49%" src="https://github-profile-summary-cards.vercel.app/api/cards/profile-details?username=saadzaveri26&theme=radical" alt="Profile Details" />
  </a>
  <a href="https://github.com/saadzaveri26">
    <img width="49%" src="https://github-profile-summary-cards.vercel.app/api/cards/productive-time?username=saadzaveri26&theme=radical&utcOffset=5.5" alt="Productive Time" />
  </a>
</p>

<p>
  <a href="https://github.com/saadzaveri26">
    <img width="32%" src="https://github-profile-summary-cards.vercel.app/api/cards/repos-per-language?username=saadzaveri26&theme=radical" alt="Repos per Language" />
  </a>
  <a href="https://github.com/saadzaveri26">
    <img width="32%" src="https://github-profile-summary-cards.vercel.app/api/cards/most-commit-language?username=saadzaveri26&theme=radical" alt="Most Commit Language" />
  </a>
  <a href="https://github.com/saadzaveri26">
    <img width="32%" src="https://github-profile-summary-cards.vercel.app/api/cards/stats?username=saadzaveri26&theme=radical" alt="Stats" />
  </a>
</p>

</div>

<br/>

<div align="center">
<img src="https://user-images.githubusercontent.com/73097560/115834477-dbab4500-a447-11eb-908a-139a6edaec5c.gif" width="100%" />
</div>

<!-- ═══════════════════════════════════════════════════════════════════════ -->
<!-- CURRENTLY EXPLORING -->
<!-- ═══════════════════════════════════════════════════════════════════════ -->

## 🌱 &nbsp;Currently Exploring

<div align="center">

<table>
<tr>
<td align="center" width="20%">
<img src="https://img.shields.io/badge/AI_Agents-fe428e?style=for-the-badge&logoColor=white" /><br/>
<sub>Autonomous task planning,<br/>tool use & reasoning</sub>
</td>
<td align="center" width="20%">
<img src="https://img.shields.io/badge/LLM_Engineering-a855f7?style=for-the-badge&logoColor=white" /><br/>
<sub>Fine-tuning, RLHF,<br/>prompt engineering</sub>
</td>
<td align="center" width="20%">
<img src="https://img.shields.io/badge/RAG_Systems-f8d847?style=for-the-badge&logoColor=black" /><br/>
<sub>Vector stores, embeddings,<br/>retrieval pipelines</sub>
</td>
<td align="center" width="20%">
<img src="https://img.shields.io/badge/Multi_Agent_Systems-a9fef7?style=for-the-badge&logoColor=black" /><br/>
<sub>Agent orchestration,<br/>collaboration protocols</sub>
</td>
<td align="center" width="20%">
<img src="https://img.shields.io/badge/Agentic_AI-ff007c?style=for-the-badge&logoColor=white" /><br/>
<sub>Autonomous workflows,<br/>self-improving agents</sub>
</td>
</tr>
<tr>
<td align="center" width="20%">
<img src="https://img.shields.io/badge/System_Design-00d4ff?style=for-the-badge&logoColor=black" /><br/>
<sub>Distributed architecture,<br/>scalability patterns</sub>
</td>
<td align="center" width="20%">
<img src="https://img.shields.io/badge/Cloud_Native-a855f7?style=for-the-badge&logoColor=white" /><br/>
<sub>Serverless, microservices,<br/>service mesh</sub>
</td>
<td align="center" width="20%">
<img src="https://img.shields.io/badge/In_Browser_IDEs-fe428e?style=for-the-badge&logoColor=white" /><br/>
<sub>WebContainers,<br/>WebAssembly runtimes</sub>
</td>
<td align="center" width="20%">
<img src="https://img.shields.io/badge/Cybersecurity-f8d847?style=for-the-badge&logoColor=black" /><br/>
<sub>Security practices,<br/>OWASP & secure coding</sub>
</td>
<td align="center" width="20%">
<img src="https://img.shields.io/badge/Workflow_Automation-00d4ff?style=for-the-badge&logoColor=black" /><br/>
<sub>n8n, Inngest,<br/>event-driven workflows</sub>
</td>
</tr>
</table>

</div>

<br/>

<div align="center">
<img src="https://user-images.githubusercontent.com/73097560/115834477-dbab4500-a447-11eb-908a-139a6edaec5c.gif" width="100%" />
</div>

<!-- ═══════════════════════════════════════════════════════════════════════ -->
<!-- LET'S CONNECT -->
<!-- ═══════════════════════════════════════════════════════════════════════ -->

## 🤝 &nbsp;Let's Connect

<div align="center">
<p>

<a href="mailto:zaverisaad26@gmail.com">
  <img src="https://img.shields.io/badge/Email-zaverisaad26%40gmail.com-EA4335?style=for-the-badge&logo=gmail&logoColor=white" alt="Email" />
</a>
&nbsp;
<a href="https://linkedin.com/in/muhammed-saad-zaveri-771875292">
  <img src="https://img.shields.io/badge/LinkedIn-Muhammed%20Saad%20Zaveri-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn" />
</a>
&nbsp;
<a href="https://github.com/saadzaveri26">
  <img src="https://img.shields.io/badge/GitHub-saadzaveri26-181717?style=for-the-badge&logo=github&logoColor=white" alt="GitHub" />
</a>

</p>
<p>

<a href="https://leetcode.com/Saadzaveri26">
  <img src="https://img.shields.io/badge/LeetCode-Saadzaveri26-FFA116?style=for-the-badge&logo=leetcode&logoColor=black" alt="LeetCode" />
</a>
&nbsp;
<a href="https://www.codechef.com/users/saadzaveri26">
  <img src="https://img.shields.io/badge/CodeChef-saadzaveri26-5B4638?style=for-the-badge&logo=codechef&logoColor=white" alt="CodeChef" />
</a>
&nbsp;
<a href="https://www.geeksforgeeks.org/user/saadzaveri26">
  <img src="https://img.shields.io/badge/GeeksforGeeks-saadzaveri26-0F9D58?style=for-the-badge&logo=geeksforgeeks&logoColor=white" alt="GFG" />
</a>

</p>
<p>

<a href="https://www.hackerrank.com/saadzaveri26">
  <img src="https://img.shields.io/badge/HackerRank-saadzaveri26-00EA64?style=for-the-badge&logo=hackerrank&logoColor=black" alt="HackerRank" />
</a>
&nbsp;
<a href="https://twitter.com/saadzaveri26">
  <img src="https://img.shields.io/badge/X_(Twitter)-saadzaveri26-000000?style=for-the-badge&logo=x&logoColor=white" alt="Twitter/X" />
</a>

</p>
</div>

<br/>

<div align="center">
<img src="https://user-images.githubusercontent.com/73097560/115834477-dbab4500-a447-11eb-908a-139a6edaec5c.gif" width="100%" />
</div>

<!-- ═══════════════════════════════════════════════════════════════════════ -->
<!-- SIGNATURE QUOTE -->
<!-- ═══════════════════════════════════════════════════════════════════════ -->

<div align="center">

<br/>

<img src="https://quotes-github-readme.vercel.app/api?type=horizontal&theme=radical" alt="Random Dev Quote" width="70%"/>

<br/><br/>

```
"The best engineers don't just write code — they architect systems that outlive them,
 solve problems that matter, and leave the codebase better than they found it."
```

<sub>— Muhammed Saad Zaveri</sub>

<br/><br/>

<!-- Support -->
<p>
  <a href="https://github.com/saadzaveri26">
    <img src="https://img.shields.io/badge/If_you_like_my_work,_give_it_a_⭐-141321?style=for-the-badge&logo=github&logoColor=fe428e" alt="Star" />
  </a>
</p>

</div>

<!-- Footer Wave -->
<img src="https://capsule-render.vercel.app/api?type=waving&color=0:0d1117,30:fe428e,70:a855f7,100:00d4ff&height=120&section=footer" width="100%" />
